...
function callListOrder() {
  ...
  $.ajax({
    ...
    success: function (response) {
      ...
      if (game == 1)
        $("#ketQua").text("next result: " + `${settings.k3d == "-1" ? "Random" : settings.k3d}`);
      if (game == 3)
        $("#ketQua").text("next result: " + `${settings.k3d3 == "-1" ? "Random" : settings.k3d3}`);
      if (game == 5)
        $("#ketQua").text("next result: " + `${settings.k3d5 == "-1" ? "Random" : settings.k3d5}`);
      if (game == 10)
        $("#ketQua").text(
          "next result: " + `${settings.k3d10 == "-1" ? "Random" : settings.k3d10}`,
        );
...
function messNewJoin5(data) {
  ...
  if (typeGame == "two-same") {
    let money =
      Number($(`#2-so-trung`).attr("totalMoney")) + Number(data.money) * Number(data.xvalue);
    $(`#2-so-trung`).attr("totalMoney", money);
    $(`#2-so-trung`).text(money);
  }
  if (typeGame == "three-same") {
    let money =
      Number($(`#3-so-trung`).attr("totalMoney")) + Number(data.money) * Number(data.xvalue);
