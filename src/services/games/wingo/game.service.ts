...
import { processWingoPayouts } from "./payout.service";
import { calculateSmartAmount, getSmartResult } from "./result.service";

/**
 * Handle Wingo game cycle
 */
export const handleWingoGame = async (db: Pool, typeId: number): Promise<void> => {
 ...
 */
export const addWingoPeriod = async (db: Pool, game: number): Promise<void> => {
  const gameMap: Record<number, string> = {
    1: "wingo",
    3: "wingo3",
    5: "wingo5",
 ...
 */
export const getPredefinedResult = async (db: Pool, game: string): Promise<string | null> => {
 ...
 */
export const calculateSmartAmountForGame = async (db: Pool, game: string): Promise<number> => {
  const currentSession = await getCurrentWingoSession(db, game);
