...
import { K3BetCalculation, K3BetRecord, K3GameJoin } from "../../../types";

/**
 * Validate bet selection based on game type
 */
export const validateBetSelection = (join: string, gameJoin: K3GameJoin): boolean => {
  switch (gameJoin) {
    case "1": // Total
  ...
  switch (betType) {
    ...
    case "two-same":
      ...
      const hasSpecificPair =
        (dice[0] === pairValue && dice[1] === pairValue) ||
    ...
    case "three-same":
      ...
      const isSpecificTriple =
        dice[0] === tripleValue && dice[1] === tripleValue && dice[2] === tripleValue;
