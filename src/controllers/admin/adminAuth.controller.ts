...
import { get5DHistory, getCurrent5DSession, update5DControlSettings } from "../../db/";
import { K5DApiResponse, K5DEditResultSchema } from "../../types/";
    ...
    try {
      ...
      const isValid = parts.every(
        (part) =>
          part === "-1" ||
          (/^\d{5}$/.test(part) &&
            part.split("").every((d) => parseInt(d) >= 0 && parseInt(d) <= 9)),
      );
...
import { findUserByPhone, updateAdminConfig } from "../../db/";
import {
  AdminApiResponse,
  AdminLoginSchema,
  AdminRegisterSchema,
...
export const createAdminAuthController = (db: Pool) => ({
  ...
  login: async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      const authToken = generateOrderIdAdmin("ADM");
      await db.execute("UPDATE users SET authToken = ? WHERE id = ?", [authToken, user.id]);
  ...
  register: async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      if (inviteCode) {
        const inviter = await db.execute("SELECT id FROM users WHERE referralCode = ?", [
          inviteCode,
        ]);
  ...
  banned: async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      await db.execute("UPDATE users SET status = ? WHERE phone = ?", [parseInt(status), phone]);
