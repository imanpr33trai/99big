...
import { getTotalJoin, updateAdminConfig } from "../../db/admin.queries";
import { EditResultSchema, TotalJoinSchema } from "../../types/admin.types";
...
export const createAdminGameController = (db: Pool) => ({
  ...
  getPhoneByInvite: async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      const [rows] = await db.execute("SELECT phone FROM users WHERE referralCode = ?", [invite]);
