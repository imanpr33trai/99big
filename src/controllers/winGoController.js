...
function formateT(params) {
  const result = params < 10 ? "0" + params : params;
  return result;
}
...
const betWinGo = async (req, res) => {
  ...
  const fee = x * money * 0.02;
  const total = x * money - fee;
  // console.log(total);
  const timeNow = Date.now();
  const check = userInfo.money - total;

  const date = new Date();
  const years = formateT(date.getFullYear());
  const months = formateT(date.getMonth() + 1);
  const days = formateT(date.getDate());
  const id_product = years + months + days + Math.floor(Math.random() * 1000000000000000);

  const formatTime = timerJoin();
  ...
  const checkTime = timerJoin(date.getTime());
...
const listOrderOld = async (req, res) => {
  const { typeid, pageno, pageto } = req.body;
  ...
  const page = Math.ceil(wingoAll.length / 10);
  return res.status(200).json({
    code: 0,
    msg: "Receive success",
    data: {
...
const GetMyEmerdList = async (req, res) => {
  ...
  const page = Math.ceil(minutes_1All.length / 10);

  const datas = minutes_1.map((data) => {
    const { id, phone, code, invite, level, game, get, ...others } = data;
...
const addWinGo = async (game) => {
  try {
    ...
    if (minPlayers.length >= 2) {
      ...
      const smallestCategory = categories.reduce(
        (smallest, category) =>
      ...
      if (availableBets.length > 0) {
        ...
        lowestBet = betsForCategory.reduce((lowest, bet) => (bet < lowest ? bet : lowest));
      }
...
const handlingWinGo1P = async (typeid) => {
  ...
  for (let i = 0; i < order.length; i++) {
    ...
    const bet = orders.bet;
    console.log("bet", bet);
    const total = orders.money;
