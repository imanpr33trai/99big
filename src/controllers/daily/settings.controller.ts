...
import { createRedEnvelope, getRedEnvelopesByCTV } from "../../db";
import { CreateBonusSchema, DailyApiResponse, SettingsSchema } from "../../types";
import { generateRedEnvelopeCode } from "../../utils";
  ...
  settings: async (req: Request, res: Response<DailyApiResponse>): Promise<void> => {
    try {
      ...
      if (!type || type === "get") {
        ...
        // Update Telegram
        await db.execute("UPDATE userPoints SET telegramId = ? WHERE userId = ?", [
          value || "",
          user.id,
        ]);
