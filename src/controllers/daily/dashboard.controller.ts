...
import {
  ...
  getUserPoints,
  getWithdrawalsByUsersDaily,
  updateCTVPoints,
  updateUserBalance,
} from "../../db";
import { BuffMoneySchema, DailyApiResponse, DateFilterSchema } from "../../types";
import { formatTimeIST, getEndOfDay, getStartOfDay, getTodayString } from "../../utils";
...
export const createCTVDashboardController = (db: Pool) => ({
  ...
  infoCtv: async (req: Request, res: Response<DailyApiResponse>): Promise<void> => {
    try {
      ...
      const f2s = await getSubordinatesByLevel(
        db,
        f1s.map((f) => f.referralCode),
      );
      const f3s = await getSubordinatesByLevel(
        db,
        f2s.map((f) => f.referralCode),
      );
      const f4s = await getSubordinatesByLevel(
        db,
        f3s.map((f) => f.referralCode),
      );
      ...
      // Calculate totals
      const allDeposits = await getDepositsByUsersDaily(db, memberIds);
      const allWithdrawals = await getWithdrawalsByUsersDaily(db, memberIds);

      const todayStart = getStartOfDay();
      const todayEnd = getEndOfDay();
      const todayDeposits = await getDepositsByUsersDaily(db, memberIds, todayStart, todayEnd);
      const todayWithdrawals = await getWithdrawalsByUsersDaily(
        db,
        memberIds,
        todayStart,
        todayEnd,
  ...
  infoCtv2: async (req: Request, res: Response<DailyApiResponse>): Promise<void> => {
    try {
      ...
      if (!validation.success) {
        res
          .status(400)
          .json({ message: "Invalid date format", status: false, timeStamp: timeNow() });
        return;
      }
      ...
      // Get filtered data
      const dayDeposits = await getDepositsByUsersDaily(db, memberIds, startOfDay, endOfDay);
      const dayWithdrawals = await getWithdrawalsByUsersDaily(db, memberIds, startOfDay, endOfDay);
      ...
      const winDay = dayBets
        .filter((b) => b.winAmount > 0)
        .reduce((sum, b) => sum + b.winAmount, 0);
      const lossDay = dayBets
        .filter((b) => b.winAmount === 0)
  ...
  buffMoney: async (req: Request, res: Response<DailyApiResponse>): Promise<void> => {
    try {
      ...
      if (!ctvPoints) {
        res
          .status(404)
          .json({ message: "CTV points not found", status: false, timeStamp: timeNow() });
        return;
      }
      ...
      if (select === "1") {
        ...
        if (ctvPoints.pointsUs < money) {
          res
            .status(400)
            .json({ message: "Insufficient balance", status: false, timeStamp: timeNow() });
          return;
        }
